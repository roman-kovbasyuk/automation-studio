...
export function ComponentDocs() {
  ...
  const groups = [
    ...
    { title: 'Basics', items: basicsPages.map(item => ({ id: item.id, label: item.title, href: `/basics.html?basic=${item.id}` })) },
  ...
  return <AtomsRoot className="docs-page">
    ...
    <header className="docs-header"><DocsBrand /><div className="docs-search" ref={searchRef}><SearchField label="Quick search" value={query} onChange={value => { setQuery(value); if (value.trim() && window.matchMedia?.('(max-width: 800px)').matches) setMobileOpen(true) }} placeholder="Find a page… /" onKeyDown={event => { if (event.key === 'Escape') setQuery('') }} /></div><div className="docs-catalog-link"><NavigationList label="Documentation" items={[{ id: 'basics', label: 'Basics', href: '/basics.html?basic=color' }, { id: 'blocks', label: 'UI Blocks', href: '/ui-blocks.html?block=sidebar' }]} /></div><div className="docs-mobile"><Drawer title="Documentation" trigger="Browse documentation" open={mobileOpen} onOpenChange={setMobileOpen}>{navigation(true)}</Drawer></div></header>
