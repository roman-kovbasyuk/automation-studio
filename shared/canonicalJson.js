...
// This module is imported by both server-side services and browser contracts.
// Keep hashing synchronous and dependency-free so response validation does not
// pull Node-only crypto into the client bundle.
function sha256(input) {
  return sha256Bytes(new TextEncoder().encode(input))
}

export function sha256Bytes(bytes) {
  if (!ArrayBuffer.isView(bytes) || bytes.BYTES_PER_ELEMENT !== 1) throw new TypeError("SHA-256 requires Uint8Array bytes")
