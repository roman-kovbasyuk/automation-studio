...
test('split panel keeps interactive filters with the heading and content separate', async () => {
  function Example() {
    ...
    return <Panel variant="split" title="Campaign" description="Choose a format" filters={<Checkbox label="Show preview" checked={show} onChange={e => setShow(e.target.checked)} />}>
      {show && <p>Campaign preview</p>}
    </Panel>
  }
  render(<Example />)
  const panel = screen.getByRole('region', { name: 'Campaign' })
  const header = panel.querySelector('header')!
  expect(header).not.toBeNull()
  expect(within(header).getByRole('heading', { name: 'Campaign' })).toBeInTheDocument()
  expect(within(header).getByText('Choose a format')).toBeInTheDocument()
  expect(header.contains(screen.getByText('Campaign preview'))).toBe(false)
  await userEvent.click(within(header).getByRole('checkbox', { name: 'Show preview' }))
  expect(screen.queryByText('Campaign preview')).not.toBeInTheDocument()
})
