...
export function ComponentDocs() {
  ...
  return <AtomsRoot className="docs-page">
    ...
    <div className="docs-shell"><aside className="docs-sidebar" aria-label="Documentation">{navigation()}</aside><main className="docs-article" key={page.id}><Container maxWidth="52rem"><Stack gap={12}>
      ...
      {page.examples ? <section id="examples"><Stack gap={6}><Heading level={2} variant="h3">Examples</Heading><Stack gap={6}>{page.examples.map(example => <section id={`example-${example.id}`} key={example.id}><CodeExample title={example.title} description={example.description} headingLevel={3} filename={`${page.id}.tsx · ${example.title}`} source={example.source} preview={<example.preview />} /></section>)}</Stack></Stack></section> : <section id="examples"><Stack gap={6}><Heading level={2} variant="h3">Examples</Heading><CodeExample title={page.example?.title ?? `${page.title} states`} description={page.example?.description ?? 'Use the same component with its supported states and props.'} filename={`${page.id}.tsx · variant example`} source={page.source} preview={<Example />} /></Stack></section>}
      {page.examples ? <section id="composition"><Stack gap={6}><Heading level={2} variant="h3">Composition</Heading><Text>Build a small product-specific wrapper while keeping Button as the shared primitive.</Text>{page.composition && <CodeExample title={page.composition.title} description={page.composition.description} filename={`${page.id}.tsx · composition`} source={page.composition.source} preview={<page.composition.preview />} />}</Stack></section> : <section id="usage"><Panel title="Usage guidance" headingLevel={2} variant="split"><Stack gap={3}>{page.notes.map(note => <Text key={note}>{note}</Text>)}</Stack></Panel></section>}
