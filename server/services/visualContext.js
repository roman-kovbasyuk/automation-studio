...
import { campaignCopyVariantSchema } from '../../shared/contracts.js'
import { rawBrief } from '../../shared/briefAnalysis.js'
import {reviewedAnalysis} from '../../shared/briefingDependencies.js'
...
export async function loadVisualContext(client, campaign, input) {
  ...
  const copies = result.rows.flatMap(set => set.candidates.filter(copy => !set.deleted_candidate_ids.includes(copy.id)))
    .map(copy => campaignCopyVariantSchema.parse(copy))
  ...
  if (!analysis.rows[0]?.analysis || !analysis.rows[0].brief
    || (campaign.brief.briefing?campaign.brief.briefing.sourceKey!==analysis.rows[0].brief.briefing?.sourceKey:hashCanonical(rawBrief(campaign.brief)) !== hashCanonical(rawBrief(analysis.rows[0].brief)))) {
    throw visualError('visual_input_required', 'Analyze the current brief to create visual prompts.')
  }
  const currentAnalysis = reviewedAnalysis(campaign.brief,campaign.brief.analysis??analysis.rows[0].analysis)
  if(campaign.brief.briefing) input={...input,context:{tags:campaign.brief.briefing.answers.visualTags,note:input.context?.note??''}}
  if (input.mode === 'campaign') return { mode: 'campaign', brief: campaign.brief, analysis: currentAnalysis, copies, ...(input.context ? { context: input.context } : {}) }
  ...
  return { mode: 'selected_copy', brief: campaign.brief, analysis: currentAnalysis, copies: selected, ...(input.context ? { context: input.context } : {}) }
}
...
export function validVisualResult(context, directions) {
  ...
  if (context.mode === 'campaign') return directions.length === 5 && directions.every(item => !item.copyId)
