...
export function openStore(dataDir) {
  ...
  async function reserveVersion(baseVersion) { return withWorkerLock(async () => { const versions = await readJson(versionsPath, {}); const match = /^([0-9]+)\.([0-9]+)\.([0-9]+)(?:-[0-9A-Za-z]+(?:[.-][0-9A-Za-z]+)*)?$/.exec(String(baseVersion)); if (!match) throw new Error('baseVersion must be semver-like'); const key = `${match[1]}.${match[2]}.${match[3]}`; const next = Number(versions[key] ?? 1); if (!Number.isSafeInteger(next) || next < 1) throw new Error('release version counter is invalid'); versions[key] = next + 1; await atomicWrite(versionsPath, versions); return `${key}-change.${next}` }) }
