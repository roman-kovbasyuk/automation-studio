...
import { FeedbackButton, Panel, Tabs } from '../components'
...
export function CodeExample({ title, description, filename, source, preview, controls, copyable = true, headingLevel = 2, className = '' }: CodeExampleProps) {
  ...
  const copyButton = copyable ? <FeedbackButton size="compact" className="b-code-example__copy" icon="copy" successIcon="check" label="Copy" successLabel="Copied" aria-label={`Copy ${title} code`} successAriaLabel={`Copied ${title} code`} onAction={copy} onError={handleCopyError} resetKey={source} /> : null
