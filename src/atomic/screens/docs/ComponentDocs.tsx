...
export function ComponentDocs() {
  ...
  const buttonExampleClass = page.id === 'button' ? 'docs-button-example' : undefined
  return <AtomsRoot className="docs-page">
    ...
    <div className="docs-shell"><aside className="docs-sidebar" aria-label="Documentation">{navigation()}</aside><main className="docs-article" key={page.id}><Container maxWidth="52rem"><Stack gap={12}>
      <section id="overview"><Stack gap={6}><Stack gap={3}><Text variant="small" tone="secondary">{page.category} / Components</Text><Heading level={1} variant="h1" id="docs-title" tabIndex={-1}>{page.title}</Heading><Text tone="secondary">{page.description}</Text></Stack><div className="docs-inline-index"><Menu label="On this page" icon="chevronDown" items={contents} onSelect={id => { window.location.hash = id }} /></div><CodeExample className={buttonExampleClass} title={`${page.title} overview`} headingLevel={2} filename={`${page.id}.tsx`} source={page.source} preview={<Preview />} /></Stack></section>
      <section id="installation"><Stack gap={6}><Heading level={2} variant="h3">Installation</Heading><Text tone="secondary">Use {page.title} from the shared package after the one-time library setup.</Text><ComponentInstallation page={page} /></Stack></section>
      {page.examples ? <section id="examples"><Stack gap={6}><Heading level={2} variant="h3">Examples</Heading><Stack gap={6}>{page.examples.map(example => <section id={`example-${example.id}`} key={example.id}><CodeExample className={buttonExampleClass} title={example.title} description={example.description} headingLevel={3} filename={`${page.id}.tsx · ${example.title}`} source={example.source} preview={<example.preview />} /></section>)}</Stack></Stack></section> : <section id="examples"><Stack gap={6}><Heading level={2} variant="h3">Examples</Heading><CodeExample className={buttonExampleClass} title={page.example?.title ?? `${page.title} states`} description={page.example?.description ?? 'Use the same component with its supported states and props.'} filename={`${page.id}.tsx · variant example`} source={page.source} preview={<Example />} /></Stack></section>}
      {page.examples ? <section id="composition"><Stack gap={6}><Heading level={2} variant="h3">Composition</Heading><Text>Build a small product-specific wrapper while keeping Button as the shared primitive.</Text>{page.composition && <CodeExample className={buttonExampleClass} title={page.composition.title} description={page.composition.description} filename={`${page.id}.tsx · composition`} source={page.composition.source} preview={<page.composition.preview />} />}</Stack></section> : <section id="usage"><Panel title="Usage guidance" headingLevel={2} variant="split"><Stack gap={3}>{page.notes.map(note => <Text key={note}>{note}</Text>)}</Stack></Panel></section>}
