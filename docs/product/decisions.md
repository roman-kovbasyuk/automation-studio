...
| D39 | **Brief review is AI-prefilled and stepwise.** One analysis fills the settings it has a basis for (age range, gender, goal, reach, the copy question and visual keywords) and marks them as suggested, never inferring age or gender from stereotypes. The requester reviews them in up to three steps: Copy found, Settings and Visual context. Found copy is always kept, with optional new copy. Image prompts must use the keywords and audience settings. See [Brief review](../specs/brief-review.md). | 17 Sep 2026 | The single review form; choosing between keeping found copy and creating new copy |
