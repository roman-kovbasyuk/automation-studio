...
try {
  ...
  if (!packageSource) {
    ...
    const packed = execFileSync('npm', ['pack', '--json', '--ignore-scripts', '--pack-destination', workspace, artifact], {
      cwd: root,
      encoding: 'utf8',
  ...
  writeFileSync(join(fixture, 'package.json'), JSON.stringify({
    ...
    dependencies: {
      'brutalist-design-system': `file:${packageSource}`,
  ...
  const installed = JSON.parse(readFileSync(join(fixture, 'node_modules', 'brutalist-design-system', 'package.json'), 'utf8'))
  if (installed.name !== 'brutalist-design-system') throw new Error('Installed tarball is not the design-system package')
  if (expectedVersion && installed.version !== expectedVersion) throw new Error(`Installed ${installed.version}; expected ${expectedVersion}`)
  run(['run', 'typecheck'])
  run(['run', 'build'])
  run(['run', 'build:ssr'])
