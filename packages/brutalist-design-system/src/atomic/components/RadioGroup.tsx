...
export type RadioGroupProps = {
  ...
  /** `tags` draws the options as wrapping, tag-shaped choices; they stay native radios. */
  variant?: 'default' | 'tags'
}
export function RadioGroup({ id, name, label, instructions, error, options, value, defaultValue, onChange, disabled = false, required, className = '', customOption, variant = 'default' }: RadioGroupProps) {
  ...
  const rows = choices.map((option, index) => <label className="c-choice-row" key={option.value} htmlFor={`${groupId}-${index}`}>
  ...
  return <fieldset id={groupId} disabled={disabled} className={`c-radio-group${tags ? ' c-radio-group--tags' : ''} ${className}`.trim()} aria-describedby={supportIds(groupId, instructions, error)} aria-invalid={hasContent(error) || undefined}>
    <legend><Text as="span" variant="h7">{label}</Text></legend>
    <Stack gap={2}>
      <FieldSupport id={groupId} instructions={instructions} />
      {tags ? <div className="c-radio-tags">{rows}</div> : <Stack gap={0}>{rows}</Stack>}
      {customOption && (!tags || selected === customOption.value) && <TextField label={`${customOption.label} answer`} name={name ? `${name}Custom` : undefined} value={customOption.text} onChange={e => customOption.onTextChange(e.target.value)} placeholder={customOption.placeholder} disabled={disabled || selected !== customOption.value} required={required && selected === customOption.value} />}
