...
test('code view exposes the same source that Copy writes', async () => {
  ...
  expect(copy).toHaveAttribute('aria-label', 'Copied Usage code')
  expect(copy.querySelector('[data-icon="check"]')).toBeTruthy()
...
test('copy feedback identifies the copied source, including completion after source changes', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Copied Install code' })).toHaveTextContent('Copied')
  expect(screen.queryByRole('status')).not.toBeInTheDocument()
  example.rerender(<CodeExample title="Install" filename="terminal" source="yarn add ./package.tgz" />)
  expect(screen.getByRole('button', { name: 'Copy Install code' })).toHaveTextContent('Copy')
})
