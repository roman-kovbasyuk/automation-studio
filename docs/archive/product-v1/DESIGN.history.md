...
> **Historical visual reference — 15 September 2026.** Read [FRONTEND.md](FRONTEND.md), especially [the three design layers](FRONTEND.md#3-three-design-layers) and [application visual language](FRONTEND.md#5-application-visual-language), before front-end work. The installed public `brutalist-design-system` package is authoritative. The local token paths, `--v2-*` scale, legacy import recommendations and universal shape rules below are superseded where they conflict; they are not implementation instructions. Do not copy these values into local CSS. Preserve this document as migration history.
