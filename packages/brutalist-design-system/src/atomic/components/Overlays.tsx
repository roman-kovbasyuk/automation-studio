...
export type MenuProps = { iconOnly?: boolean; icon?: IconName; side?: 'top' | 'bottom'; label: string; items: readonly MenuItem[]; onSelect: (id: string) => void; disabled?: boolean; className?: string }
...
export function Menu({ label, items, onSelect, disabled, iconOnly, icon='more', side='bottom', className = '' }: MenuProps) { const triggerClassName = `c-menu-trigger ${className}`.trim(); return <M.Root><M.Trigger asChild>{iconOnly ? <Button className={triggerClassName} variant="quiet" iconOnly aria-label={label} icon={icon} disabled={disabled}/> : <Button className={triggerClassName} icon={icon} disabled={disabled}>{label}</Button>}</M.Trigger><M.Portal><AtomsRoot className="c-overlay-root"><M.Content side={side} className="c-popup c-menu" sideOffset={8} collisionPadding={16} aria-label={label}>{items.map(item => { const itemIcon = item.icon ?? menuIcons[item.id]; return <M.Item className="c-menu__item" key={item.id} disabled={item.disabled} data-danger={item.danger || undefined} onSelect={() => onSelect(item.id)}>{itemIcon && <Icon name={itemIcon} size="small" />}<Text as="span" variant="small" tone="inherit">{item.label}</Text></M.Item> })}</M.Content></AtomsRoot></M.Portal></M.Root> }
