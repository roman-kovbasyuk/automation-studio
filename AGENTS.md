...
The Brutalist package owns UI appearance and shared interaction. Application patterns own composition and domain behavior. Published brand systems own artwork and generated assets. Read [DESIGN.md](DESIGN.md) for the current visual application guide. Do not infer current product behavior from archived documents when they conflict with FRONTEND.md and the current product documentation.

## UI implementation rules

- The `brutalist-design-system` package is the single source of UI components and tokens. It lives in this repository as the detachable workspace package `packages/brutalist-design-system` (D36, [design system integration](docs/specs/design-system-integration.md)). Import only `brutalist-design-system` and `brutalist-design-system/styles.css`; local adapters may translate app callbacks/props without changing component appearance or interaction.
- Brutalist code lives only in `packages/brutalist-design-system` and follows its [AGENTS.md](packages/brutalist-design-system/AGENTS.md). It never imports application code or undeclared modules and keeps product terms (*Automation Studio*, *campaign*, *banner*, *recipe*, *escalation*) out of its source unless `scripts/design-system-vocabulary.json` lists a reviewed exception.
- Do not target package component classes, redefine package tokens, or reproduce component skins through local classes/inline styles. Plain app-owned containers may arrange content. `npm run design-system:check` enforces the workspace link, package isolation, public entry points, product vocabulary, named imports, private CSS selectors, token definitions and explicit JSX className/style props on direct package imports; manual review still covers indirect wrappers and spread props.
- Check the package exports and the [gap list](docs/design-system/missing-components.md) before writing local UI. Decide where a missing need belongs ([design system integration](docs/specs/design-system-integration.md#where-a-need-belongs)): generic needs are added to the package in the same pull request, with package tests, accessibility coverage and a documentation example; product-specific compositions stay in the application. Existing adapters in `src/components/design-system/` are migration debt, not permission to add new ones.
- Additive package changes need no extra approval and bump the package's patch version. Breaking changes (a removed or renamed export, a changed required prop) need the owner's approval and a minor version bump (D25). Run `npm run verify --workspace brutalist-design-system` when package files change.
