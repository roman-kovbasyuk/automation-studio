...
export function DistributeView({ input = {}, access = {}, operation = {}, actions = {}, expectedInputKey }) {
  ...
  return <section aria-label="Distribute module" aria-busy={pending || undefined}>
    ...
    <div className="bs-review-summary"><div><StatusBadge>Version {input.version.versionNumber}</StatusBadge><h3>{input.delivery ? 'Ready to download' : 'Approved and ready to export'}</h3></div></div>
    <Surface><div className="bs-delivery"><CheckCircle2 size={32} aria-hidden="true"/><div><p>The package contains the approved PNG banners{input.version.snapshot.videos?.length ? ', selected MP4 videos' : ''} and a manifest identifying the exact files.</p>
