...
- Check the installed exports and the [gap list](docs/design-system/missing-components.md) before writing local UI. When a required interaction is absent upstream, use native browser defaults, record the gap and decide where the need belongs ([design system integration](docs/specs/design-system-integration.md)). Existing adapters in `src/components/design-system/` are migration debt, not permission to add new ones.
- **Never edit Brutalist from the app.** Request design-system changes through the Brutalist change protocol. The owner's standing approval covers additive requests from this app; breaking changes need owner approval. Every release must pass Brutalist verification and the app's install checks (`npm run design-system:update`) before use.
