...
export type FileDropzoneProps = { label: string; onFiles: (files: File[]) => void; accept?: string; multiple?: boolean; maxSize?: number; disabled?: boolean; state?: 'idle' | 'uploading' | 'success' | 'error'; progress?: number; error?: string; onRetry?: () => void; size?: 'default' | 'small' }
export function FileDropzone({ label, onFiles, accept, multiple = false, maxSize = 10 * 1024 * 1024, disabled, state = 'idle', progress = 0, error: externalError, onRetry, size = 'default' }: FileDropzoneProps) {
