...
import { Inline, Stack } from '../atoms'
...
export function SearchField({ label, value, onChange, ...props }: SearchFieldProps) { return <Stack gap={2} className="c-search-field"><TextField {...props} type="search" label={label} value={value} onChange={event => onChange(event.target.value)} /><Inline><Button size="compact" icon="close" disabled={props.disabled || !value} onClick={() => onChange('')}>Clear {label}</Button></Inline></Stack> }
