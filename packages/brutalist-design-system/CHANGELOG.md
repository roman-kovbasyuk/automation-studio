...
- Added `TagInput`: free-text tags added with Enter, styled as a dashed, tag-shaped entry control inline with the existing tags (closes gap R7). Typed text is trimmed, deduplicated case-insensitively and length-limited; Backspace on an empty entry removes the last tag; blur commits a pending entry.
