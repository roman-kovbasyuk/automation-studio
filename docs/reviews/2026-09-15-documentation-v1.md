...
The complete-coverage pass adds `/getting-started.html`, a generated index of every Basics page, grouped component destination and UI-block destination. It is the Introduction entry in each docs sidebar and the target of the catalog’s Documentation v1 link.

## Delivered

- `basics.html`: eight Basics pages, retained as the foundation reference.
- `components.html`: grouped component documentation. Thirty-eight destination pages cover all current exported component names, including the existing grouped families Select/Combobox/MultiSelect, Tabs/SegmentedControl and Tag/StatusBadge.
- `ui-blocks.html`: Sidebar panel, AI Prompt Input and Code Example UI-block pages.
- `getting-started.html`: complete documentation index for all published elements.
...
- `npm run build:atomic`: passed with root, semantic documentation entries, and numbered compatibility redirects.
