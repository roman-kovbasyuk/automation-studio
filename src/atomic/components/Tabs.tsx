...
export type TabsProps = { label: string; items: readonly TabItem[]; value?: string; onChange?: (value: string) => void; listOnly?: boolean; className?: string } | SegmentedControlProps
export function Tabs(props:TabsProps) { const [internal,setInternal]=useState('items' in props ? props.items.find(i=>!i.disabled)?.id : undefined)
 ...
 const {label,items,value,onChange,listOnly=false,className=''}=props,selected=value??internal,index=items.findIndex(i=>i.id===selected)
