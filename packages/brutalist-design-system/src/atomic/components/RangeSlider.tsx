...
/** Two independently labelled native controls on one shared scale; values are clamped so the handles never cross. */
export function RangeSlider({ label, value, onChange, min = 0, max = 100, names, lowerLabel = 'Minimum', upperLabel = 'Maximum', instructions, className = '', ...props }: RangeSliderProps) {
  ...
  return <fieldset className={`c-range-slider ${className}`.trim()} disabled={props.disabled}>
    ...
    <Stack gap={4}>
      ...
      <Slider {...props} label={lowerLabel} name={names?.[0]} min={min} max={upper} value={low} onChange={next => onChange([Math.min(next, high), high])} />
      <Slider {...props} label={upperLabel} name={names?.[1]} min={min} max={upper} value={high} onChange={next => onChange([low, Math.max(next, low)])} />
    </Stack>
  </fieldset>
}
