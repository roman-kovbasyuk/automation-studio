...
export function TextField({ label, instructions, error, id, className = '', type = 'text', width = ['number','time','date','color'].includes(type) ? 'content' : 'full', trailing, onChange, 'aria-describedby': external, 'aria-invalid': invalid, ...props }: TextFieldProps) {
  ...
  return <Stack gap={2} className={`c-field${width === 'content' ? ' c-field--content' : ''}`} style={{ '--field-characters': characters + 2 } as CSSProperties}>
    ...
    <div className="c-text-input-wrap"><input {...props} onChange={event => { setLength(event.target.value.length); onChange?.(event) }} id={fieldId} type={type} className={`c-text-input ${className}`.trim()} aria-describedby={supportIds(fieldId, instructions, error, external)} aria-invalid={hasContent(error) || invalid || undefined} />{trailing && <span className="c-text-input__trailing">{trailing}</span>}</div>
