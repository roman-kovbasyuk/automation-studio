...
- [x] Record the current `codex/docs-basics` status and save the new files and dirty edits in a checkpoint commit once Git escalation is available.
- [x] Merge the committed `main` history into this isolated worktree and resolve conflicts in package metadata, README, scripts and shared docs by keeping the latest protocol implementation plus the current docs/components work.
- [x] Re-run the three baseline checks and record any pre-existing warning separately.
...
- [x] Move the metadata registry and update every importer to the new UI-independent path.
- [x] Add parity checks that compare registry names with public component exports and documented destinations without hardcoding a permanent count.
- [x] Replace the old catalog integration test with a current documentation behavior test while retaining component unit coverage.
- [x] Replace `/atomic.html` with a small compatibility redirect for known legacy anchors and a Getting Started fallback.
- [x] Remove catalog-only files only after `rg` confirms no imports, Vite inputs, docs links, or package references remain.
...
- [x] Move usage guidance to the stable path and preserve the raw-import parser contract.
- [x] Add visible root links to Basics, Components, and UI Blocks; keep the existing page-20/21/22/23 URLs.
- [x] Ensure every page's preview and copyable source describe the same supported variant; label fragments that are not standalone modules.
- [x] Document current limitations honestly, including CommandMenu, DigitInput, Avatar fallback, TabMenuVertical, ProgressRing, Toast/Notification and application-owned table behavior.
- [x] Remove links to `.worktrees` and the obsolete catalog from repository guidance.
...
- [x] Add the 20 currently absent public components to the independent consumer without changing their APIs.
- [x] Make the consumer verification derive expected names from the UI-independent registry and assert all exports are rendered or imported.
- [x] Add the example verifier and `test:changes` to the umbrella verification command without creating worker recursion.
- [x] Run the consumer with isolated npm cache and dependencies; reject symlink-only or network-skipped validation.

The existing source-level documentation test covers every Components and UI Blocks source module; the public-package production verifier continues to compile all eight Basics modules and their 16 named exports.
...
- [x] Preserve the latest `main` recovery, release, lock and rollback behavior during merge.
- [x] Allow only `src/atomic/**`, exact atomic tests, `fixtures/atomic-consumer/**`, and the canonical usage guide where a component change needs it; continue rejecting dependencies, release tooling, arbitrary docs, and app files.
- [x] Add allowed-path and forbidden-path tests, including `fixtures/atomic-consumer` and the stable guidance path.
- [x] Verify idempotent requests, conflicts, `working`/`ready`/`failed`, immutable package paths, app adoption, rollback and concurrent dependency-file protection.
...
- [x] Fold useful decisions from dated plans/reviews into the permanent documents, then remove only superseded current-tree copies.
- [x] Run `rg` audits for old catalog paths, `.worktrees` links, private source imports, stale route promises, and old component counts.
- [x] Run the complete verification matrix: boundary tests, atomic tests, typecheck, docs examples, build, library build, fresh consumer, and change protocol tests.
- [x] Review the final diff and commit in small checkpoints. Report commit IDs and any runtime limitation separately.
