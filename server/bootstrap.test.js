...
describe('production server composition', () => {
  test('selects Gemini in production without startup migrations and closes every owned resource', async () => {
    const calls = []
    ...
    const dependencies = {
      openStaticBuild: vi.fn(async () => { calls.push('open-static'); return staticBuild }),
    ...
    expect(staticBuild.close).toHaveBeenCalledOnce()
  ...
  test('rejects a missing production build before allocating database or provider resources', async () => {
    ...
    const dependencies = {
      openStaticBuild: vi.fn(async () => { throw new Error('Production static build is unavailable; run npm run build before startup') }),
      createPool,
    }
  ...
  test('rejects a production startup-migration override before allocating any resource', async () => {
    const createPool = vi.fn()
    const openStaticBuild = vi.fn()
    await expect(createServerRuntime({
      environment: {
        NODE_ENV: 'production', DATABASE_URL: 'postgresql:///banner_studio', FIREBASE_PROJECT_ID: 'banner-project',
        GENERATION_PROVIDER: 'gemini', VERTEX_AI_PROJECT_ID: 'banner-project',
      ...
      dependencies: { createPool, openStaticBuild },
    })).rejects.toThrow('RUN_MIGRATIONS cannot be enabled in production')
    expect(openStaticBuild).not.toHaveBeenCalled()
    expect(createPool).not.toHaveBeenCalled()
  })
